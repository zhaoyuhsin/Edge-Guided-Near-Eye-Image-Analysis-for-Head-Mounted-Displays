...
class DenseNet2D_up_block(nn.Module):
    ...
    def forward(self, prev_feature_map, e_prev_feature_map, x):
        x = F.interpolate(x,
                          mode='bilinear',
                          align_corners=False,
                          scale_factor=self.up_stride)
        x = torch.cat([x, prev_feature_map, e_prev_feature_map], dim=1)
...
class DenseNet_decoder(nn.Module):
    def __init__(self, setting, chz, out_c, growth, actfunc=F.leaky_relu, norm=nn.BatchNorm2d):
        ...
        ipSize = [306, 115, 76, 38]
        opSize = [115, 76, 38, 32]
        self.up_block4 = DenseNet2D_up_block(skipSize[0] * 2, ipSize[0], opSize[0], 2, actfunc)
        self.up_block3 = DenseNet2D_up_block(skipSize[1] * 2, ipSize[1], opSize[1], 2, actfunc)
        self.up_block2 = DenseNet2D_up_block(skipSize[2] * 2, ipSize[2], opSize[2], 2, actfunc)
        self.up_block1 = DenseNet2D_up_block(skipSize[3] * 2, ipSize[3], opSize[3], 2, actfunc)

        self.final = convBlock(chz, chz, out_c, actfunc)

    def forward(self, skip4, skip3, skip2, skip1, e_skip4, e_skip3, e_skip2, e_skip1, x):
        x = self.up_block4(skip4, e_skip4, x)
        x = self.up_block3(skip3, e_skip3, x)
        x = self.up_block2(skip2, e_skip2, x)
        x = self.up_block1(skip1, e_skip1, x)
        o = self.final(x)
        return o
...
class DenseNet2D(nn.Module):
    ...
    def forward(self,
        ...
        assert(self.setting['add_edge'] == 1)

        e_x4, e_x3, e_x2, e_x1, e_x = self.enc(x_edge)
        x = torch.cat((x, e_x), 1)

        op = self.dec(x4, x3, x2, x1, e_x4, e_x3, e_x2, e_x1, x)
